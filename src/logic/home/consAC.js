...
export class ConsAC extends ConsBase {
  constructor() {
    ...
    this.consAddSet = ["consACcool", "consACheat"];
  }
   ...
   */
  equip(year, size) {
    let sizeThreshold = [2.8, 4.5, 100]; //last is maxsize
  ...
  calcMeasure() {
    let mes; //temporary letiable of measure instance

    //mACreplace
    mes = this.measures["mACreplace"];
    if (!this.isSelected("mACreplaceHeat")) {
      if (D6.consListByName["consACcool"][0].electricity > 0 && D6.consListByName["consACcool"][1].electricity > 0 && this.subID == 0) {
        ...
        if (this.acHeat.heatArea > 0.3 && this.acHeat.houseSize > 60) {
          let num = Math.round(Math.max(this.acHeat.houseSize * this.acHeat.heatArea, 25) / 25);
    ...
    if (this.acHeat.heatEquip != 1) {
      ...
      if (this.heatArea > 0.3 && this.houseSize > 60) {
