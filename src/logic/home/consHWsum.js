...
export class ConsHWsum extends ConsBase {
  ...
  calc() {
    ...
    this.tanklossEnergy =
      this.tankLossWatt / 1000 * Unit.calorie.electricity * 365 / 12;
    ...
    switch (this.equipType) {
      ...
      default:
        this.mainSource = "gas";
        this.gas =
          this.heatEnergy / this.performanceEcojozu / Unit.calorie.gas;
        this.performanceOrg = this.performanceEcojozu;
    }
  ...
  //		none
  // set
  //		calclate result in this.measures[] also link to this.measuresList[]
  calcMeasure() {
    ...
    // 230203 fix
    let endEnergyNow = this.jules * 1000 / 4.18 * this.performanceOrg;
    if (
      this.equipType < 5 &&
      !goodPerformance
    ) {
      ...
      if (this.equipType == 3) {
        ...
        this.measures["mHWecofeel"].kerosene =
          endEnergyNow / this.performanceEcojozu / Unit.calorie.kerosene;
      ...
      this.measures["mHWecojoze"].gas =
        endEnergyNow / this.performanceEcojozu / Unit.calorie.gas;
    ...
    if (
      ...
      let notCoGenerationEnergy = Math.min(endEnergyNow, 500 * 1000 / 12); //	kcal/month
      let coGenerationEnergy = endEnergyNow - notCoGenerationEnergy;
      ...
      let baseGas = 500 * 1000 / 12; //	kcal/month
      ...
      if (typeof this.measures["mHWenefarmSOFC"] !== 'undefined') {
        ...
        let generateElectricity = Math.min(400, D6.consTotal.electricity * 0.7);
        notCoGenerationEnergy = 0; //	kcal/month
        let availableEnergy = 25 * 2 * (80 - 20) * 30; //	kcal/month 25L*2,60K
        coGenerationEnergy = endEnergyNow - notCoGenerationEnergy;

        this.measures["mHWenefarmSOFC"].gas =
          (
            generateElectricity * Unit.calorie.electricity / this.performanceEnefarmSOFCEle +
    ...
    let rejectSolarSelect = false; //can or not to install solar heater
    if (
      this.isSelected("mHWecocute") ||
      this.isSelected("mHWhybrid") ||
      this.isSelected("mHWenefarm") ||
