...
export class ConsHTsum extends ConsBase {
  ...
  precalc() {
    ...
    //window 
    let heatWindow = [6.5, 1.4, 1.8, 3.6, 4.65, 6.5, 6.5];
    let nowHeatWindow = heatWindow[this.performanceWindow];
    if (this.heatArea <= 2 && nowHeatWindow > 5) {
      //cold area default
      nowHeatWindow = heatWindow[4];
    }
    let windowrate = 0.48;								// heat loss through window
  ...
  calc() {
    ...
    let heatArea_m2 = this.houseSize * this.heatSpace;
    if (this.heatSpace > 0.05) {
      heatArea_m2 = Math.max(heatArea_m2, 13);		//minimum 13m2
    }

    //calculate heat energy
    let heatKcal = this.calcHeatLoad(heatArea_m2, this.heatTime, this.heatMonth, this.heatTemp);
    ...
    //calculate residue
    let elecOver = 0;
    let coef = (this.heatEquip == 1 ? this.apf : 1);
    if (this.heatEquip == 1 || this.heatEquip == 2) {
      if (this.priceEleWinter > 0) {
        let priceMaxCons = this.priceEleWinter * 0.7
          / Unit.price.electricity
          * this.heatMonth / 12;
        if (heatKcal / coef / Unit.calorie.electricity > priceMaxCons) {
          //in case that calculated electricity is more than fee
          elecOver = heatKcal - priceMaxCons * coef * Unit.calorie.electricity;
          heatKcal -= elecOver;
        }
      }
    }
    ...
    this.calcACkwh = heatKcal / this.apf / Unit.calorie.electricity;
    if (this.mainSource == "electricity" && this.heatEquip < 2) {
      //set air conditioner
      this[this.mainSource] = this.calcACkwh;
    } else {
      //other than air conditioner
      this[this.mainSource] = heatKcal / Unit.calorie[this.mainSource];
    }
    ...
    //electricity
    consbyprice = Math.max(0, this.priceEleWinter - this.priceEleSpring) / Unit.price.electricity;
    ...
    //gas
    consbyprice = Math.max(0, this.priceGasWinter - this.priceGasSpring) / Unit.price.gas;
    //let gasover = 0;
    ...
    if (this.consKeros != -1 && this.hotwaterEquipType != 3 && this.hotwaterEquipType != 4) {
      consbyprice = this.consKeros / Unit.price.kerosene;
    ...
    if ( elecOver > 0 ) {
      //kerosene fix
      if (Unit.areaHeating <= 4 && this.priceKeros > 0 ) {
        this.kerosene +=  elecOver *  Unit.calorie.electricity /Unit.calorie.kerosene;
      } else {
        this.gas +=  elecOver *  Unit.calorie.electricity /Unit.calorie.gas;
      }
    }
    //gas over fix
    if ( gasover>0 ){
      if( this.priceKeros > 0 ) {
        this.kerosene = Math.min( gasover * Unit.calorie.gas / Unit.calorie.kerosene, this.consKeros / Unit.price.kerosene );
      } else {
        this.electricity +=  gasover * Unit.calorie.gas / Unit.calorie.electricity/ this.apf;
      }
    }

    //kerosene use estimate is more than fee
    if (keroseneover>0){
      this.electricity +=  keroseneover * Unit.calorie.kerosene / Unit.calorie.electricity/ this.apf;
    }
    ...
    */
    ...
    heatKcal = this.electricity * Unit.calorie.electricity * nowapf
      + this.gas * Unit.calorie.gas
      + this.kerosene * Unit.calorie.kerosene;
    this.endEnergy = heatKcal;
  ...
  calc2nd() {
    let spaceK;
    let spaceG;
    let consHW = D6.consShow["HW"];
    let consTotal = D6.consShow["TO"];
    let consCK = D6.consShow["CK"];
    ...
    //this.electricity += this.consListByName["consHWtoilet"][0].electricity;
  ...
  calcHeatLoad(heatArea_m2, heatTime, heatMonth, heatTemp) {
    ...
    let heatLoadUnit = this.heatLoadUnit_Wood;
  ...
  calcMeasure() {
    ...
    if (this.houseType != 2
      ...
      let heatlossfactor = 1 / (this.performanceWall + 50);
      let newlossfactor = 1 / (150 + 50);
